...
$rating_width = @number_format($current_rating/$count,2)*$rating_unitwidth;$rating_width = @number_format($current_rating/$count,2)*$rating_unitwidth;
...
if ($static == 'static') {
		...
		$static_rater[] .= '</ul>';
		$static_rater[] .= '<p style="padding-top: 4px"><h2>Promedio: <strong> '.$rating1.'</strong> ('.$count.' '.$tense.') </h2></p>';
		$static_rater[] .= '</div>';
		$static_rater[] .= '</div>'."\n\n";
      ...
      for ($ncount = 1; $ncount <= $units; $ncount++) { // loop from 1 to the number of units
           if(!$voted) { // if the user hasn't yet voted, draw the voting stars
              $rater.='<li><a href="db.php?j='.$ncount.'&amp;q='.$id.'&amp;t='.$ip.'&amp;c='.$units.'" title="'.$ncount.'" class="r'.$ncount.'-unit rater" rel="nofollow">'.$ncount.'</a></li>';
           }
      }
      ...
      $rater.='  </ul>';
      $rater.='  <p';
      if($voted){ $rater.=' class="voted"> Ya haz votado.'; }
      $rater.='  </p>';
      $rater.='</div>';
      $rater.='</div>';
      return $rater;
...
function static_rating_bar($id, $units, $fill) {

		require('_config-rating.php');

		$rating_width = @number_format($fill,2)*$rating_unitwidth;
